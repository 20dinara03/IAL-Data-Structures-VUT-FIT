...
void List_Init( List *list ) 
{
	...
	list->activeElement = NULL; //set the active element to NULL
}
...
{
	struct ListElement *Next_element, *Active_element; //Initializing variables for the element we are working on and the next one
	Active_element = list->firstElement;               //We assign the value of the very first element to the element we are working on

	while (Active_element != NULL)                     //If first elemen is not NULL (if the list is not empty)
	{                                                   
		Next_element = Active_element->nextElement;    //We sequentially assign the value of the next element, 
		free(Active_element);                          //clear the active element, 
		Active_element = Next_element;                 //and replace it with the next one
	}
...
void List_InsertFirst( List *list, int data ) 
{
	...
	if (list->firstElement == NULL)
	{
	 ...
	 list->firstElement = new_node;
	}
	else
	{
     new_node->nextElement = list->firstElement;
	 list->firstElement = new_node;
	}
}
...
{
	struct ListElement *Delete_element = list->firstElement;
...
void List_DeleteAfter( List *list ) 
{
	...
	struct ListElement *after = list->activeElement->nextElement;
    struct ListElement *after_delete = after->nextElement;
	...
	{
		list->activeElement->nextElement = after_delete;
	}
...
{
  struct ListElement *Insert_after = malloc(sizeof(struct ListElement));
